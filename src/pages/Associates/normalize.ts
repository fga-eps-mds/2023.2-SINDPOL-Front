...
export function normalizeAndCreateObject(row: RowData): { data: any; error?: string } {
  ...
  for (const key in row) {
    if (row.hasOwnProperty(key)) {
      const value = row[key as keyof RowData]; // Acessa o valor do campo
      const isValid = validateField(key, value); // Valida o campo

      if (!isValid) {
        errorFields.push(key); // Adiciona o campo à lista de erros
      }
    }
  }
  // Se houver campos inválidos, retorna o objeto com indicador de erro
  if (errorFields.length > 0) {
    return {
      data: normalizedData,
      error: `Os seguintes campos são inválidos: ${errorFields.join(', ')}`,
    };
  }

  // Se não houver erros, retorna apenas o objeto normalizado
